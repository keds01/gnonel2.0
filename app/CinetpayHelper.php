...
class CinetpayHelper
{
    public static  function generatePaymentLink($transactionId, $amount, $description, $returnUrl, $cancelUrl, User $user)
    {
        $client = new Client([
            'timeout' => 30,
            'verify' => false,  // Désactiver SSL verify pour test local
            'curl' => [
        ...
        $payload = [
            ...
            'customer_name' => $user->name ?: 'Client', // Client's lastname
            'customer_surname' => ($user->prenom && strlen($user->prenom) >= 2) ? $user->prenom : 'Gnonel', // Client's firstname
            'customer_phone_number' => $user->telephone ?: '00000000', // Client's phone
