...
class User extends Authenticatable
{
    ...
    {
        // Vérifier uniquement les abonnements actifs (statut = 1)
        $souscription = DB::table('souscriptions')
            ->where('iduser', '=', $user->id)
            ->where('statut', '=', 1) // Uniquement les abonnements payés et actifs
        ...
        if ($souscription == null) {
            $souscription = DB::table('souscriptions')
                ->where('iduser', '=', $user->pack_user_id)
                ->where('statut', '=', 1) // Uniquement les abonnements payés et actifs
                ->join('abonnement', 'abonnement.id', '=', 'souscriptions.idabonnement')
                ->orderBy('souscriptions.idsouscription', 'DESC')
                ->first();
    ...
    {
        // Try different ways to load TCPDF
        if (!class_exists('TCPDF')) {
            ...
            if (file_exists($tcpdfPath)) {
                ...
                throw new \Exception('TCPDF library is not installed. Please run: composer require tecnickcom/tcpdf');
            }
        }
        ...
        $pdf->SetCreator('Gnonel');
        $pdf->SetAuthor('Gnonel');
        $pdf->SetTitle('Gnonel - Rapport');
        $pdf->SetSubject('Rapport Gnonel');
        $pdf->SetKeywords('Gnonel, Rapport, PDF');
