...
class FrontController extends Controller
{
  ...
  public function viewmesref()
  {
    ...
    if ($verif == null && Auth::user()->type_user == 3) {
      ...
      if ($verif->date_fin == null) {
        return redirect(route('welcome_abonne'));
  ...
  public function detailsreference($id)
  {
    ...
    \Log::info('=== DÉBUT VÉRIFICATION ABONNEMENT OPÉRATEUR ===', $debug_info);
    ...
    \Log::info('Vérification des utilisateurs associés à l\'opérateur', array_merge($debug_info, [
      ...
      'utilisateurs_ids' => $users_ref->pluck('id')->toArray()
    ]));
    ...
    foreach ($users_ref as $user) {
      // Utiliser la méthode verifabonnement pour vérifier l'abonnement de chaque utilisateur
      $user_abonnement = User::verifabonnement($user);
      
      $user_info = [
        'user_id' => $user->id,
        'name' => $user->name,
        'email' => $user->email,
        'date_fin' => $user_abonnement ? $user_abonnement->date_fin : 'NULL',
        'abonnement_complet' => $user_abonnement ? json_encode($user_abonnement) : 'NULL'
      ];
      
      // Un abonnement est valide si date_fin existe et est dans le futur
      if ($user_abonnement && $user_abonnement->date_fin !== null && $user_abonnement->date_fin >= date('Y-m-d')) {
        $abonnement_valide = true;
        $user_info['abonnement_valide'] = true;
        \Log::info('ABONNEMENT VALIDE TROUVÉ pour utilisateur', $user_info);
      } else {
        $user_info['abonnement_valide'] = false;
        if (!$user_abonnement) {
          $user_info['raison'] = 'Pas d\'abonnement trouvé';
        } elseif ($user_abonnement->date_fin === null) {
          $user_info['raison'] = 'Pas de date de fin';
        } else {
          $user_info['raison'] = 'Abonnement expiré';
        }
        \Log::info('ABONNEMENT INVALIDE pour utilisateur', $user_info);
      }
    ...
    if (!$abonnement_valide) {
      \Log::info('Aucun abonnement valide trouvé pour cet opérateur - AFFICHAGE MODAL', $debug_info);
      return back()->with('add_ok', '');
    }
    ...
    \Log::info('=== FIN VÉRIFICATION ABONNEMENT OPÉRATEUR ===', $debug_info);
  ...
  {
    return view('landing.services.basic');
  }
  public function service_options()
  {
    // Page archivée - temporairement inaccessible
    abort(404, 'Page non disponible pour le moment');
  }
  ...
  public function listspecabonne()
  {
    ...
    if ($verif == null) {
      // Utilisateur sans abonnement actif - rediriger vers la page d'abonnement
      session()->flash('message', 'Veuillez souscrire à un abonnement pour accéder à cette fonctionnalité.');
      return redirect(route('pricing'));
    } else {
      // Utilisateur avec abonnement actif - vérifier la date d'expiration
      if ($verif->date_fin == null) {
        return redirect(route('welcome_abonne'));
      } elseif ($verif->date_fin < date('Y-m-d')) {
        session()->flash('message', sprintf('Veuillez vous reabonner votre abonnement etait expiré le ' . Carbon::parse($verif->date_fin)->format('d/m/Y')));
  ...
  public function annuler($id)
  {
    ...
    if (!$souscription) {
      return redirect()->back()->with('flash_message_error', 'Souscription non trouvée.');
    }
  ...
  public function filtrerspec(Request $request)
  {
    try {
      ...
      $recherche = $request->input('motcle');
      
      // Log pour déboguer les valeurs reçues
      \Log::info('Filtrage des specs - Requête reçue', [
      ...
      $verif = User::verifabonnement(Auth::user());
      \Log::info('Vérification abonnement utilisateur', ['verif' => $verif]);
      ...
      $total = $query->count();
      \Log::info('Nombre total de résultats trouvés', ['total' => $total]);
      ...
      // Log de la réponse
      \Log::info('Réponse générée pour la recherche de specs', [
