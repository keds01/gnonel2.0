...
Route::get('return-subscription', function (Request $request) {
    $status = $request->get('status', 'unknown');
    $transactionId = $request->get('id');

    Log::info('FedaPay - Retour paiement', ['status' => $status, 'id' => $transactionId, 'params' => $request->all()]);

    if ($status === 'approved') {
        return redirect()->route('welcome_abonne')->with('flash_message_success', 'Paiement réussi ! Votre abonnement est maintenant actif.');
    } elseif ($status === 'canceled') {
        return redirect()->route('pricing')->with('flash_message_error', 'Paiement annulé.');
    } else {
        return redirect()->route('welcome_abonne')->with('flash_message_info', 'Statut du paiement : ' . $status);
    }
})->name('return-subscription');

Route::get('cancel-subscription', function (Request $request) {
    Log::info('FedaPay - Paiement annulé par utilisateur', ['params' => $request->all()]);
    
    // Récupérer l'ID de transaction depuis les paramètres
    $transactionId = $request->get('id') || $request->get('transaction_id');
    
    if ($transactionId) {
        // Extraire l'ID de souscription depuis le transaction ID
        $parts = explode('_', $transactionId);
        $souscriptionId = $parts[0] ?? null;
        ...
        if ($souscriptionId) {
            ...
            Log::info('FedaPay - Souscription marquée comme annulée', ['souscription_id' => $souscriptionId]);
        }
    }
    
    return redirect()->route('pricing')->with('flash_message_error', 'Paiement annulé. Vous pouvez réessayer quand vous voulez.');
...
// Route GET redirigée vers pricing - utiliser /Souscription/Abonnement/{offre} pour s'inscrire
Route::get('/souscription', function () {
    return redirect()->route('pricing');
})->name('choix_souscription');

// Route POST conservée pour traiter le formulaire d'inscription
...
Route::get('/offre-abonnements', 'FrontController@pricing')->name('choix_abonnement');
...
Route::get('services/basic', 'FrontController@service_basic')->name('service-basic');
...
// Route archivée - Page Options temporairement inaccessible
...
Route::group(['middleware' => ['auth', 'role:user']], function () {
    ...
    Route::get('Details/{reference}', 'FrontController@detailsreference')->name('details_short');
    ...
    Route::get('/recherche-offreappel', 'RechercheController@rechercheoffre')->name('rechercheoffre');
    Route::get('search-offre/{id?}', 'RechercheController@rechercheajax')->name('search_offre_ajax');
    Route::get('listspecabonne', 'FrontController@listspecabonne')->name('listspecabonne');
    // Route unique pour le filtrage des spécifications
    ...
    // Routes pour les autorités contractantes - Publication d'appels d'offres
    Route::get('/autorite/appel-offre/creer', 'RechercheController@createOffreForm')->name('autorite.offre.create');
    Route::post('/autorite/appel-offre/store', 'RechercheController@storeOffre')->name('autorite.offre.store');
    Route::get('/autorite/mes-appels-offres', 'RechercheController@mesOffres')->name('autorite.mes.offres');
    Route::get('/autorite/appel-offre/modifier/{id}', 'RechercheController@editOffre')->name('autorite.offre.edit');
    Route::post('/autorite/appel-offre/update/{id}', 'RechercheController@updateOffre')->name('autorite.offre.update');
    Route::get('/autorite/appel-offre/supprimer/{id}', 'RechercheController@deleteOffre')->name('autorite.offre.delete');
    
    // Route de diagnostic pour vérifier l'abonnement d'un opérateur
    Route::get('/debug-operateur-abonnement/{operateur_id}', function($operateur_id) {
        $operateur = DB::table('operateurs')->where('id', $operateur_id)->first();
        if (!$operateur) {
            return response()->json(['error' => 'Opérateur non trouvé'], 404);
        }
        
        $users = DB::table('users')->where('ratache_operateur', $operateur_id)->get();
        $utilisateurs_details = [];
        $est_abonne = false;
...
// Route temporaire pour convertir mixinter_vf en autorité contractante
Route::get('/convertir-autorite/{email}', function($email) {
    ...
    return 'Utilisateur ' . $email . ' converti en autorité contractante (ID autorité: ' . $autorite_id . ')';
})->middleware('auth');
