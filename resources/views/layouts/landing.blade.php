...
<body class="body">
    ...
    @php
        $hasFlashError = false;
        $hasFlashSuccess = false;
        $hasFlashWarning = false;
        $flashError = null;
        $flashSuccess = null;
        $flashWarning = null;
        ...
        try {
            if (Session::has('flash_message_error')) {
                $hasFlashError = true;
                $flashError = session('flash_message_error');
            }
            if (Session::has('flash_message_success')) {
                $hasFlashSuccess = true;
                $flashSuccess = session('flash_message_success');
            }
            if (Session::has('flash_message_warning')) {
                $hasFlashWarning = true;
                $flashWarning = session('flash_message_warning');
            }
        } catch (\Exception $e) {
            // Ignorer les erreurs de session/DB
        }
    @endphp
    
    @if ($hasFlashError)
        <script type="text/javascript">
            swal("{{ $flashError }}", "Merci", "error");
        </script>
    @endif
    @if ($hasFlashSuccess)
        <script type="text/javascript">
            swal("{{ $flashSuccess }}", "Merci", "success");
        </script>
    @endif
    @if ($hasFlashWarning)
        <script type="text/javascript">
            swal("{{ $flashWarning }}", "Merci", "warning");
        </script>
    @endif
    @yield('scripts')
