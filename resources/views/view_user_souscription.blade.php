                                    ...
                                    <table>
                                        <tbody>
                                            ...
                                            <tr>
                                                <td class="p-1">
                                       @php
    // Pour les packs fixes, utiliser prix_gros directement
    if ($souscriptions->is_pack_fixe) {
        $prixTotal = $souscriptions->prix_gros ?? 0;
        $prixAPayer = $prixTotal;
    } else {
        // Ancien calcul pour les packs normaux
        $prixTotal = $souscriptions->montant_finale_apaye + ($souscriptions->discount_pack ?? 0) + ($souscriptions->discount_recom ?? 0);
        $prixAPayer = $souscriptions->montant_finale_apaye;
    }
@endphp
<tr>
    <td class="p-1">Prix total</td>
    <td class="p-1"><b>{{ number_format($prixTotal, 0, '.', ' ') }} FCFA</b></td>
</tr>
@if (!$souscriptions->is_pack_fixe && $souscriptions->discount_recom != null && $souscriptions->discount_recom > 0)
<tr>
...
@endif
@if (!$souscriptions->is_pack_fixe && $souscriptions->discount_pack != null && $souscriptions->discount_pack > 0)
<tr>
...
<tr>
    ...
    <td class="p-1"><b style="font-size:1.1em">{{ number_format($prixAPayer, 0, '.', ' ') }} FCFA</b></td>
</tr>
                                            </tr>
                                            <tr>
                                    ...
                                    <div class="col-lg-12">
                                        <a href="{{ route('valider_souscription', $souscriptions->idsouscription) }}"
                                            ...
                                            style='background-color: #3232cc; border-bottom-color: #3232cc; color:#ffffff; display: inline-block;'
                                            id="btn-paiement">Procéder au paiement</a>
                                        <a class="btn btn-danger btn-sm mt-3"
                                            href="{{ route('annuler_abonnement', $souscriptions->idsouscription) }}"
                                            style="display: inline-block;">Annuler le paiement</a>
                                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            console.log('Page chargée - bouton paiement prêt');
            
            var btnPaiement = document.getElementById('btn-paiement');
            if (btnPaiement) {
                btnPaiement.addEventListener('click', function(e) {
                    console.log('Clic sur Procéder au paiement - URL:', this.href);
                    // Laisser le comportement par défaut du lien se faire
                });
            }
        });
    </script>
